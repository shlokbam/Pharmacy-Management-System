...
function updateMedicine($id, $name, $packing, $generic_name, $suppliers_name) {
  ...
  // Insert or update medicine in the stock table
  $check_stock_query = "SELECT * FROM medicines_stock WHERE NAME = '$name'";
  $stock_result = mysqli_query($con, $check_stock_query);

  // If no stock entry exists, create one with default values
  if (mysqli_num_rows($stock_result) == 0) {
    $insert_stock_query = "INSERT INTO medicines_stock (NAME, BATCH_ID, EXPIRY_DATE, QUANTITY, MRP, RATE) VALUES ('$name', 'DEFAULT_BATCH', '00-00-0000', 0, 0, 0)";
    mysqli_query($con, $insert_stock_query);
  }

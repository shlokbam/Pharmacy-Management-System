...
function printInvoice($invoice_number) {
    require "db_connection.php";
    if ($con) {
        $query = "SELECT * FROM invoices INNER JOIN customers ON invoices.CUSTOMER_ID = customers.ID WHERE invoices.INVOICE_ID = $invoice_number"; // Updated this line
        $result = mysqli_query($con, $query);
        ...
        $query = "SELECT * FROM invoices WHERE INVOICE_ID = $invoice_number"; // Updated this line
        $result = mysqli_query($con, $query);
        $row = mysqli_fetch_array($result);
        if (!$row) {
            echo "Invoice not found.";
        ...
        <div class="col-md-10 table-responsive">
            <table class="table table-bordered table-striped table-hover" id="purchase_report_div">
                <tbody>
                    ...
                    $seq_no = 0;
                    $total = 0;
                    $query = "SELECT * FROM medicines_stock WHERE ID = $invoice_number"; // Updated this line
                    $result = mysqli_query($con, $query);
